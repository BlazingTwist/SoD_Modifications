...
public class BTConfigUtils
{
	...
	private class ConfigNode
	{
		...
		public override string ToString() {
			...
			builder.Append("Key: ");
			builder.Append((key == null) ? "null" : key);
	...
	private static void ParseList(ConfigNode node, Type type, IList list) {
		...
		foreach(ConfigNode entry in node.listValues) {
			if(entry.value != null) {
				...
				throw new InvalidOperationException("got node without any values! key = " + (node.key == null ? "null" : node.key));
			}
		}
	}
